...
void CXTPCalendarTimeLineViewGroup::AdjustLayout(CDC* pDC, const CRect& rcGroup)
{
	...
	for (i = 0; i < nCount; i++)
	{
		CXTPCalendarTimeLineViewEvent* pViewEv = m_arEvents.GetAt(i);
		if (pViewEv)
		{
	...
	for (i = 0; i < nCount; i++)
	{
		...
		for (int m = 1; m < nAr; m++)
		{
			UINT j = ar.GetAt(m);
			if (j == 0) continue;
			CXTPCalendarTimeLineViewEvent* pViewEv2 = m_arEvents.GetAt(j);
			if (pViewEv2)
			{
...
// https://forum.codejock.com/forum_posts.asp?TID=13465&PID=46020#46020
						for (int mm = m + 1; mm < nAr; mm++)
						{
							UINT jj = ar.GetAt(mm);
							if (jj == 0) continue;
							CXTPCalendarTimeLineViewEvent* pViewEv3 = m_arEvents.GetAt(jj);
...
void CXTPCalendarTimeLineViewGroup::Populate(CXTPCalendarEvents* pEvents)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		CXTPCalendarTimeLineViewEvent* pViewEv = new CXTPCalendarTimeLineViewEvent(pEv, this);
...
CXTPCalendarTimeLineView* CXTPCalendarTimeLineViewEvent::GetView()
{
...
CXTPCalendarTimeLineViewPart* CXTPCalendarTimeLineViewEvent::GetPart()
{
