...
//if we follows Outlook spec and keep fixed vert pos!
	...
	datePicker.GetPaintManager()->EnableTheme(FALSE);

	if (ct == xtpCalendarThemeResource)
	{
		CXTPDatePickerThemeOffice2007* pTheme = new CXTPDatePickerThemeOffice2007();
		if (pTheme)
		{
			pTheme->SetPaintTheme(ct);
			datePicker.SetTheme(pTheme);
		}
	}
	else
	{
