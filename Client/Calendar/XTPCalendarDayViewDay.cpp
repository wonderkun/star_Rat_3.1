...
void CXTPCalendarDayViewDay::Populate(COleDateTime dtDayDate)
{
	...
	int nViewGroupsCount = m_arViewGroups.GetCount();
	if (nViewGroupsCount > 1)
	{
		for (int i = 0; i < nViewGroupsCount; i++)
		{
			CXTPCalendarDayViewGroup* pGroup = m_arViewGroups.GetAt(i);
			if (pGroup)
			{
									...
									if (iOwnId != jSch)
									{
										for (int I = 0; I < (int) arScheduleIDs.GetSize(); I++)
										{
											if (jSch == (int) arScheduleIDs.GetAt(I))
											{
												...
												if (jGr != i)
												{
													CXTPCalendarDayViewGroup* pGroup1 = m_arViewGroups.GetAt(jGr);
													if (pGroup1)
													{
		...
		for (int iD = 0; iD < m_arViewGroups.GetCount(); iD++)
		{
			CXTPCalendarDayViewGroup* pGroup = m_arViewGroups.GetAt(iD);
			if (pGroup)
			{
				if (pGroup->m_bHide)
				{
