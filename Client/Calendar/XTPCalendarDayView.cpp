...
BOOL CXTPCalendarDayView::HitTestEx(CPoint pt, XTP_CALENDAR_HITTESTINFO_DAY_VIEW* pHitTest)
{
	...
	BOOL bShowExpandButton = XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), bShowAllDayExpandButton, TRUE);
...
//TRACE(_T("iGrCnt=%d iResCnt=%d\n"), iGrCnt, iResCnt);
		...
		if (GetScrollBarInfoH(&si, &nScrollStep))
		{
			...
			if (nChar == VK_LEFT)
			{
				if (!m_bScrollH_Disabled || dtNSel > dtFirstDay)
				{
			...
			{
				if (!m_bScrollH_Disabled || dtNSel < dtLastDay)
				{
					CXTPCalendarDayViewDay* pDay = GetDay(dtNewSelEnd);
					if (!bVKShift && pDay)
					{
