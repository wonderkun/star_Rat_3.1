...
void CXTPDatePickerTheme::SetControl(CXTPDatePickerControl*  pControl)
{
	...
	if (pControl && XTPResourceImages() && XTPResourceImages()->GetConnection())
	{
		m_Sink.Advise(XTPResourceImages()->GetConnection(), XTP_NC_COMMON_RESOURCEIMAGES_CHANGED,
			&CXTPDatePickerTheme::OnEvent_ResourceImagesChanged);
	}
}

void CXTPDatePickerTheme::OnEvent_ResourceImagesChanged(XTP_NOTIFY_CODE Event, WPARAM wParam, LPARAM lParam)
{
											 ...
											 COLORREF clrColor, BOOL bYears)
{
	...
	TBase::DrawScrollTriangle(pDC, rcSpot, bLeftDirection, clrColor, bYears);
}
...
void CXTPDatePickerTheme::CTOMonthHeader::Draw(CDC* pDC, CXTPDatePickerItemMonth* pMonth)
{
	...
	if (!rcLeftScroll.IsRectEmpty())
	{
		...
		if (pMonth->GetShowLeftScroll())
			...
			m_clrButtons, pControl->m_bYearsTriangle);
	}
...
{
	ASSERT(ePaintTheme == xtpCalendarThemeResource);
