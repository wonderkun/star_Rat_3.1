...
void CXTPCalendarCaptionBarTheme::AdjustLayout(CDC* pDC, const CRect& rcRect)
{
	...
	int nNextY = rc.top;
	BOOL b = IsSwitchViewButtons();
	BOOL bb = IsScrollDateButtons();
	BOOL bbb = IsExpandButton();
	BOOL bbbb = IsDateLabel();

	m_pButtonViewDay->SetVisible(b && !m_pButtonViewDay->GetText().IsEmpty());
	m_pButtonViewWeek->SetVisible(b && !m_pButtonViewWeek->GetText().IsEmpty());
	m_pButtonViewMonth->SetVisible(b && !m_pButtonViewMonth->GetText().IsEmpty());

	//int iWid1 = int(83 * m_dScale);
	//int iWid2 = int(20 * m_dScale);
	//int iWid3 = int(5 * m_dScale);
	...
	if (!m_bIsOneLine) //TWO LINE MODE
	{
		...
		m_pScrollPrevDate->SetVisible(bb);
		m_pScrollNextDate->SetVisible(bb);
		...
		m_pExpandButton->SetVisible(bbb);

		int nRightX = rcClient.right;
		if (bbb)
		{
			int nDelta = (bb || bbbb) ? 5 : 3;
			rc.top = nNextY + 13;
