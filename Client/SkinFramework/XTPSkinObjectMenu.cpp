...
void CXTPSkinPopupMenuState::FilterMenuKey(LPARAM lParam)
{
	...
	if (m_pRootPopup->m_bSysMenu && m_pRootPopup->m_pState == 0)
	{
	...
	for (int nIndex = 0; nIndex < nCount; nIndex++)
	{
		...
		else
		{
			...
			else if (hbm)
			{
				...
				{
					CXTPImageManagerIcon::DrawAlphaBitmap(&dc, hbm, CPoint((bTheme ? 16 : 11) -  bmpInfo.bmWidth / 2 , (rcItem.top + rcItem.bottom - bmpInfo.bmHeight) / 2),
