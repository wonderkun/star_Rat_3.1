...
void CXTPStatusBarPane::OnLButtonDown(CPoint point)
{
	...
	while (::GetCapture() == m_pStatusBar->GetSafeHwnd())
	{
		MSG msg;
...
#ifdef _XTP_INCLUDE_RIBBON
		if (point.y >= rc.bottom - 3 && GetPaintManager()->BaseTheme() == xtpThemeRibbon && CXTPOffice2007FrameHook::m_nMsgQueryFrameHook != 0)
		{
			CXTPOffice2007FrameHook* pFrame = (CXTPOffice2007FrameHook*)::SendMessage(::GetParent(m_hWnd), CXTPOffice2007FrameHook::m_nMsgQueryFrameHook, 0, 0);
...
BOOL CXTPStatusBar::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (m_pMarkupContext)
	{
		...
		{
			CXTPStatusBarPane* pPane = GetPane(i);
			if (pPane->IsVisible() && pPane->GetMarkupUIElement())
			{
				if (XTPMarkupRelayMessage(pPane->GetMarkupUIElement(), message, wParam, lParam, pResult))
					return TRUE;
			}
		}
	}
