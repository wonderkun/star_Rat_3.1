...
//              static BOOL         ms_bUseCustomHeap;  // Define does allocator use custom heap or
//              default heap;
//          };
// See Also:
...
//===========================================================================
#	define XTP_DECLARE_HEAP_ALLOCATOR(allocatorClass)                                             \
		XTP_DECLARE_HEAP_ALLOCATOR_(allocatorClass, XTP_EXPORT_PARAMS_NO)
...
// Parameters:
//      allocatorClass  - [in] Name of the allocator class.
//      bUseCustomHeap  - [in] Set as TRUE to enable custom heap by default for this allocator and
//      FALSE to disable.
// Remarks:
...
public:
	...
	static void* AFX_CDECL AllocData(size_t nSize)
	{
		if (!m_pFreeBlocks)
		{
			int nObjSize	   = _Round4(XTPToIntChecked(nSize)
									 + XTPToIntChecked(sizeof(XTP_BATCHALLOC_OBJ_HEADER)));
			int nBlockDataSize = XTPToIntChecked(sizeof(XTP_BATCHALLOC_BLOCK_HEADER)
												 + m_nBlockSize * nObjSize);

			BYTE* pBlockData = (BYTE*)_TAllocator::Alloc_mem(XTPToSizeTChecked(nBlockDataSize));
...
#	ifdef _DEBUG
			pBlockHdr->m_dwObjCount = XTPToDWORDChecked(m_nBlockSize);
...
#	endif
	...
	static void AFX_CDECL FreeData(void* pObj)
	{
		...
		if (pObjHdr->pBlockHeader->m_dwRefs == 0 && m_bDestroyEmptyBlocksOnFree)
		{
			...
			XTP_BATCHALLOC_BLOCK_HEADER* pIsNextFree = pEmptyBlock->pPrev ? pEmptyBlock->pPrev
																		  : pEmptyBlock->pNext;
...
//                          {
//                              static BOOL m_bEnableBatchAllocation;   // Define is Batch
//                              Allocation enabled; static LONG m_dwAllocatedObjects;       //
//                              allocated blocks count;
//
//                              static BOOL m_bDestroyEmptyBlocksOnFree;    // if TRUE - completely
//                              free blocks will be deallocated on free objects, otherwise they will
//                              stay in free list. static BOOL m_bDestroyLastEmptyBlockOnFree; // if
//                              TRUE - last completely free block will be deallocated on free
//                              objects, otherwise it will stay in free list.
//
//                              static int  m_nBlockSize;               // Count of objects in
//                              block.
//                          protected:
//                              static XTP_BATCHALLOC_BLOCK_HEADER* m_pFreeBlocks;  // List of
//                              blocks which have free pieces. static XTP_BATCHALLOC_BLOCK_HEADER*
//                              m_pBusyBlocks;  // List of blocks which have not free pieces.
//                          };
//
// Example:
// <code>
...
#	define XTP_DECLARE_BATCH_ALLOC_OBJ_DATA_(dataClass, EXPORT_PARAMS)                            \
		...
		{                                                                                          \
			...
			static int m_nBlockSize;                                                               \
                                                                                                   \
			static BOOL AFX_CDECL IsDataEmpty()                                                    \
			{                                                                                      \
				return !m_pFreeBlocks && !m_pBusyBlocks;                                           \
			};                                                                                     \
                                                                                                   \
		protected:                                                                                 \
...
//===========================================================================
#	define XTP_DECLARE_BATCH_ALLOC_OBJ_DATA(dataClass)                                            \
		XTP_DECLARE_BATCH_ALLOC_OBJ_DATA_(dataClass, XTP_EXPORT_PARAMS_NO)
...
//      batchAllocEnabled   - [in] Set as TRUE to enable Batch allocation by default for this object
