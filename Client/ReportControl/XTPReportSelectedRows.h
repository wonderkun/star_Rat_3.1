...
protected:
	...
	int m_nPosSelected;
	BOOL m_bNotifyOnClear; // If TRUE, then calling CXTPReportSelectedRows::Select and SelectBlock
						   // will send xtpReportSelectionClear notificatoins when the selected rows
						   // are internally cleared (normal behavior), if FALSE it will not send
						   // the clear notifications during CXTPReportSelectedRows::Select and
						   // SelectBlock

	CArray<SELECTED_BLOCK, SELECTED_BLOCK&> m_arrSelectedBlocks;

	BOOL m_bChanged;
	XTPReportRowType m_nRowType; // Only rows of one type can be selected (i.e. body, header, or
								 // footer).
								 //}}AFX_CODEJOCK_PRIVATE
