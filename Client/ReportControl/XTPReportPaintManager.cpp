	...
	if (!bDrawn)
	{
		...
		while (pos > - 1)
		{
			...
			if (pos > - 1)
			{
				...
				CXTPReportColumn* pColumn = pColumns->Find(iCol);
				pos = sSource.Find(_T("["));
				if (pos > - 1)
...
CSize CXTPReportPaintManager::DrawCollapsedBitmap(CDC* pDC, const CXTPReportRow* pRow, CRect& rcBitmap)
{
	if (m_columnStyle == xtpReportColumnResource)
	{
		...
		if (pImage)
		{
			...
			CSize sizeGlyph = NULL;

			POINT ptIcon;
			ptIcon.x = rcBitmap.left + 1;
			...
			sizeGlyph = CSize(pImage->GetWidth(), pImage->GetWidth());
