...
void CXTPChartControl::OnPaint()
{
	...
	if (m_pContent->Has3DDiagram())
	{
		if (m_pCommand && m_pDrawThread && m_pDrawThread->m_bImageReady)
		{
			CDC memDC;
			memDC.CreateCompatibleDC(&dc);

			CBitmap* pOldBitmap = memDC.SelectObject(&m_pDrawThread->m_bmpCache);
		...
		else
		{
			...
			CXTPChartDeviceContext* pDC = m_pContent->CreateDeviceContext(this, dc, rc, TRUE);
			...
			if (pDC)
			{
				if (!m_pCommand)
				{
					...
					if (!m_pDrawThread)
					{
						...
						m_pDrawThread->m_pControl = this;
						m_pDrawThread->ResumeThread();
					}

					m_pDrawThread->StartDraw(rc.Size(), m_pCommand);
				}
	...
	else if (m_bmpCache.GetSafeHandle() == 0)
	{
