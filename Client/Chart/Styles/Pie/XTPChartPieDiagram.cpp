...
void CXTPChartPieSeriesView::CalculatePointLayout(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	CXTPChartEllipse ellipse(CXTPChartDiagramPoint(), m_pDomain->GetInnerBounds().Width / 2, m_pDomain->GetInnerBounds().Height / 2);
...
CXTPChartDeviceCommand* CXTPChartPieSeriesView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	CXTPChartPie pie(0, CXTPChartMathUtils::PI / 2, explodeEllipse, 0, 0);
	...
	{
		CXTPChartColor clrBorder = GetStyle()->GetContent()->GetAppearance()->GetPieStyleAppearance()->BorderColor;
...
void CXTPChartPieSeriesPointView::CalculateLayout(double& dStartAngle)
{
	...
	CXTPChartEllipse ellipse(CXTPChartDiagramPoint(), size.Width / 2, size.Height / 2);
	...
	if (m_pPoint->m_bSpecial)
	{
		...
		CXTPChartDiagramPoint explodeLocation(explodeX, explodeY);
	...
	//CXTPChartDiagramPoint center = m_pPie->CalculateCenter(basePoint);

	//CXTPChartRectF((float)center.X - size.Width / 2,  (float)center.Y - size.Height / 2, size.Width, size.Height);
...
CXTPChartDeviceCommand* CXTPChartPieSeriesPointView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	{
		CXTPChartColor clrBorder = GetSeriesView()->GetStyle()->GetContent()->GetAppearance()->GetPieStyleAppearance()->BorderColor;
	...
	: CXTPChartDiagramView(pDiagram, pParent)
{
	...
	m_pLabelsView = new CXTPChartLabelsView(this);
