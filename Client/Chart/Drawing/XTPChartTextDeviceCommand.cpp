...
void CXTPChartTextDeviceCommand::ExecuteOverride(CXTPChartOpenGLDeviceContext* pDC)
{
	//glFinish();
	ExecuteInternal(pDC);
}
...
{
	//GdipSetTextRenderingHint(pDC->GetGraphics(), TextRenderingHintClearTypeGridFit);
	//GdipSetTextRenderingHint(pDC->GetGraphics(), TextRenderingHintAntiAlias);
	//GdipSetTextRenderingHint(pDC->GetGraphics(), TextRenderingHintSingleBitPerPixelGridFit);
	//GdipSetTextRenderingHint(pDC->GetGraphics(), TextRenderingHintClearTypeGridFit);
	...
	GdipDrawString(pDC->GetGraphics(), XTP_CT2CW(m_strText), m_strText.GetLength(), pGpFont, (RectF*)&m_rect, pGpStringFormat, pGpBrush);
