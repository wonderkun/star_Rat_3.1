...
BEGIN_DISPATCH_MAP(CXTPChartAxisLabel, CXTPChartElement)
	...
	DISP_PROPERTY_EX_ID(CXTPChartAxisLabel, "Format", 5, OleGetFormat, OleSetFormat, VT_BSTR)
...
BSTR CXTPChartAxisLabel::OleGetFormat()
{
	return m_strFormat.AllocSysString();
}

void CXTPChartAxisLabel::OleSetFormat(LPCTSTR lpszFotmat)
{
	SetFormat(lpszFotmat);
}
