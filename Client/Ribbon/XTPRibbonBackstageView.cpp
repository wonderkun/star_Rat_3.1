...
void CXTPRibbonBackstageView::SetFocusedControl(CXTPControl* pFocusedControl)
{
	if (m_pFocusedControl != pFocusedControl)
	{
		...
		if (pFocusedControl && !IsCommand(pFocusedControl))
		{
...
void CXTPRibbonBackstageView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if (nChar == VK_DOWN)
	{
		...
		nIndex = (nIndex + 1) % GetControlCount();
	...
	else if (nChar == VK_UP)
	{
		...
		if (nIndex == -1)
			nIndex = GetControlCount() - 1;
		else
			nIndex = (nIndex - 1 + GetControlCount()) % GetControlCount();
...
void CXTPRibbonBackstageView::DrawControl(CDC* pDC, CXTPControl* pControl)
{
	...
	if (IsCommand(pControl))
	{
		if (m_pHighlightedControl == pControl || m_pFocusedControl == pControl)
		{
			...
			if (pImage)
			{
				pImage->DrawImage(pDC, pControl->GetRect(), pImage->GetSource(0, 3), CRect(2, 2, 2, 2));
			}
		}
		...
		if (pIcon)
		{
			pIcon->Draw(pDC, CPoint(rcText.left, (rcText.top + rcText.bottom - pIcon->GetHeight()) / 2));
	...
	{
		if (m_pHighlightedControl == pControl || m_pActiveTab == pControl)
		{
			CXTPResourceImage* pImage = XTPResourceImages()->LoadFile(_T("BackstageMenuPage"));
			if (pImage)
			{
				pImage->DrawImage(pDC, pControl->GetRect(), pImage->GetSource(m_pActiveTab == pControl ? 1 : 0, 4), CRect(2, 2, 2, 2));
			}
...
void CXTPRibbonBackstageView::RecalcLayout()
{
	...
	if (pWnd)
	{
		...
		CRect rcContent(m_nMenuWidth, 2, rc.right, rc.bottom);
