...
public:
	CXTPRibbonBarControlCaptionButton(CXTPOffice2007FrameHook* pFrame)
	{
		m_pFrame = pFrame;
		m_bTemporary = TRUE;
...
void CXTPRibbonBar::AddCaptionButton(int nId, BOOL bAdd, BOOL bEnabled, CRect& rcCaption)
{
	...
	if (!pButton && bAdd)
	{
		pButton = m_pControls->Add(new CXTPRibbonBarControlCaptionButton(m_pFrameHook), nId, NULL, -1, TRUE);
...
{
	return m_pFrameHook && m_pFrameHook->IsDwmEnabled() ? m_pFrameHook->GetFrameBorder() : 0;
}
