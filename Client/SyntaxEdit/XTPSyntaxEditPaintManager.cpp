											  ...
											  int nLine, CXTPSyntaxEditCtrl* pEditCtrl)
{
	...
	BOOL bBreakText = pEditCtrl->m_mapRowColor.Lookup(nTextRow, crBreakText);

	COLORREF crBreakBack;
	BOOL bBreakBack  = pEditCtrl->m_mapRowBkColor.Lookup(nTextRow, crBreakBack);
	...
	int nSelStart = pEditCtrl->m_Selection.GetSelStartForRow_str(nTextRow, nLine);
	int nSelEnd = pEditCtrl->m_Selection.GetSelEndForRow_str(nTextRow, nLine, &bInfinitSelEnd);
...
//          crText = m_clrValues.crText;
		...
		if (pEditCtrl->m_Selection.IsIntersectSel_str(nTextRow, txtBlk.nPos/* + 1*/, txtBlk.nNextBlockPos) &&
