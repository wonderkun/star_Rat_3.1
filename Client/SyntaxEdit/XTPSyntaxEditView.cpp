...
{
	XTP_EDIT_NMHDR_ENABLESCROLLBAR* pNMHDR_ESCR = (XTP_EDIT_NMHDR_ENABLESCROLLBAR*)pNMHDR;

	CSplitterWnd *pSplitterWnd = GetEditCtrl().IsCreateScrollbarOnParent() ? GetParentSplitter(this, TRUE) : NULL;
	if (pSplitterWnd)
	{
		DWORD dwStyle = (DWORD)pSplitterWnd->GetScrollStyle();
		DWORD dwStyleNew = 0;

		if ((pNMHDR_ESCR->dwScrollBar & WS_VSCROLL) && GetVertScrollBar())
			dwStyleNew |= pNMHDR_ESCR->dwState & WS_VSCROLL;
		else
			dwStyleNew |= dwStyle & WS_VSCROLL;

		if ((pNMHDR_ESCR->dwScrollBar & WS_HSCROLL)  && GetHorzScrollBar())
			dwStyleNew |= pNMHDR_ESCR->dwState & WS_HSCROLL;
		else
			dwStyleNew |= dwStyle & WS_HSCROLL;

		if (dwStyleNew != dwStyle)
		{
			pSplitterWnd->SetScrollStyle(dwStyleNew);
