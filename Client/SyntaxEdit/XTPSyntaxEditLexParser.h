...
enum XTPSyntaxEditLexParseResult
{
	...
	xtpEditLPR_Iterated   = 0x010, // Indicate that current text position has already moved to the
								   // next char inside a function call.
	xtpEditLPR_TBpop1 = 0x020,	 // Indicate that new found text block should be inserted before
								   // previous one and as a parent of it.
	xtpEditLPR_Error = 0x100, // Iterated parse process breaked due to some error. See trace for
							  // details.
	xtpEditLPR_RunFinished = 0x200, // Run iterated parse process (or some big part of it) is
									// finished.
	xtpEditLPR_RunBreaked = 0x400,  // Run iterated parse process (or some big part of it) is
									// breaked.
...
protected:
	...
	CXTPSyntaxEditLexTextBlockPtr m_ptrLastChild; // for internal use only. not always contains
												  // correct value
};
...
public:
	...
	virtual void StartParseInThread(CXTPSyntaxEditBufferManager* pBuffer,
									const XTP_EDIT_LINECOL* pLCStart = NULL,
									const XTP_EDIT_LINECOL* pLCEnd = NULL, int eEdinAction = 0,
