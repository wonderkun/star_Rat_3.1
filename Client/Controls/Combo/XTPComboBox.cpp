...
void CXTPComboBox::RefreshMetrics()
{
	...
	m_wrapperComboBox.OpenTheme(0, L"COMBOBOX");
...
void CXTPComboBox::OnPaint()
{
	...
	else if (m_bUseVisualStyle && m_wrapperComboBox.IsAppThemed())
	{
		CComboBox::DefWindowProc(WM_PAINT, (WPARAM)dcMem.m_hDC, 0);

		if (!m_wrapperComboBox.IsAppThemeReady())
		{
			...
			m_wrapperComboBox.DrawThemeBackground(dcMem, 0, nState, rc, NULL);
			...
			m_wrapperComboBox.DrawThemeBackground(dcMem, CP_DROPDOWNBUTTON, nState, rcDropDownButton, NULL);
...
{
	return (m_nTheme != 0) || (m_bUseVisualStyle && m_wrapperComboBox.IsAppThemed());
}
