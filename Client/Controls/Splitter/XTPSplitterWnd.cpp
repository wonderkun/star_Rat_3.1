...
: m_pTheme(NULL)
{
	...
	m_nHiddenRow = -1;
	m_point = CPoint(-1, -1);
	m_dwxStyle = XTP_SPLIT_NOFULLDRAG;
	m_bFlatSplit = TRUE;
	m_cxSplitter = 6;
	m_cySplitter = 6;
	m_cxSplitterGap = 6;
...
BOOL CXTPSplitterWnd::SetTheme(XTPControlTheme eTheme)
{
	switch (eTheme)
	{
		case xtpControlThemeOfficeXP:
			return SetTheme(new CXTPSplitterWndThemeOfficeXP());

		case xtpControlThemeOffice2003:
			return SetTheme(new CXTPSplitterWndThemeOffice2003());

		case xtpControlThemeResource:
			return SetTheme(new CXTPSplitterWndThemeResource());
	}

	return SetTheme(new CXTPSplitterWndTheme());
}
...
LRESULT CXTPSplitterWnd::OnPrintClient(WPARAM wParam, LPARAM /*lParam*/)
{
...
void CXTPSplitterWnd::OnSettingChange(UINT uFlags, LPCTSTR lpszSection)
{
	...
	if (!m_bForceFullDrag)
	{
		...
		if (m_dwxStyle & XTP_SPLIT_NOFULLDRAG)
		{
			m_bFullDrag = FALSE;
		}
	}
...
void CXTPSplitterWnd::DrawTracker(const CRect& rect, CBrush* pBrush)
{
	if (m_bFullDrag == FALSE)
	{
		...
		if (m_dwxStyle & XTP_SPLIT_DOTTRACKER)
		{
			CRect rc;
			GetInsideRect(rc);
...
void CXTPSplitterWnd::OnMouseMove(UINT nFlags, CPoint point)
{
	if (m_bFullDrag == FALSE)
	{
...
{
	// If XTP_SPLIT_NOSIZE style is defined, just return a border
	// hit so can at least activate the app if needed.
	if ((m_dwxStyle & XTP_SPLIT_NOSIZE) == XTP_SPLIT_NOSIZE)
	{
		return HTBORDER;
	}
...
void CXTPSplitterWnd::OnDrawSplitter(CDC* pDC, ESplitType nType, const CRect& rectArg)
{
	...
	case splitBorder:
		if (m_bFlatSplit)
		{
			if (!(m_dwxStyle & XTP_SPLIT_NOBORDER))
			{
...
void CXTPSplitterWnd::OnLButtonUp(UINT nFlags, CPoint point)
{
