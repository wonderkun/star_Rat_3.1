...
void CXTPResize::SetResize(const UINT nID, const HWND hWnd, const XTP_RESIZERECT& rrcSizing)
{
	...
	{
		HWND hWndChild = m_arrItems[i]->m_hWnd;
		if (hWndChild && ::IsWindow(hWndChild) && (::GetDlgCtrlID(hWndChild) == (int)nID))
		{
			pItem = m_arrItems[i];
		}
	}
	...
	if (pItem == 0)
	{
		...
		if (hWndChild == NULL)
		{
			hWndChild = ::GetDlgItem(m_pWnd->m_hWnd, nID);
		}
		...
		pItem = new CXTPResizeItem(hWndChild, rrcSizing, rcWindow);
	...
	if (m_arrItems.GetSize() > 0)
	{
		...
		for (i = 0; i < m_arrItems.GetSize(); i++)
		{
			if (m_arrItems[i]->m_bIsGroupBox)
				::InvalidateRect(m_arrItems[i]->m_hWnd, NULL, TRUE);
		}
	}
}
...
{
	CXTPResizeItem* pItem = new CXTPResizeItem(pWnd->m_hWnd, rrcSizing, rcWindow);
	m_arrItems.Add(pItem);
}
...
void CXTPResize::UpdateControlRect(CWnd* pWnd)
{
	...
	{
		if (m_arrItems[i]->m_hWnd == pWnd->m_hWnd)
		{
			CRect rect;
...
CXTPResizeItem::CXTPResizeItem(HWND hWnd, const CXTPResizeRect& rrcSizing, CRect& rcWindow)
: m_hWnd(hWnd)
...
bool CXTPResizeItem::MakeTransparent(CXTPResize* pXTPResize)
{
	...
	{
		TCHAR szClassName[8];
		::GetClassName(m_hWnd, szClassName, 8);
		...
		if ((dwStyle & (BS_GROUPBOX | WS_TABSTOP)) == BS_GROUPBOX)
		{
			...
			if (pWnd && pWnd->IsKindOf(RUNTIME_CLASS(CXTPResizeGroupBox)))
			{
				return false;
			}
