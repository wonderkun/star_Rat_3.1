...
void CXTPEdit::RefreshMetrics()
{
	...
	m_wrapperEdit.OpenTheme(0, L"EDIT");
}
...
void CXTPEdit::DrawNcBorders(CDC* pDC, CRect rc)
{
	...
	if (m_bUseVisualStyle && m_wrapperEdit.IsAppThemed())
	{
		CRect rcClient(rc.left + 1, rc.top + 1, rc.right - 1, rc.bottom - 1);
		pDC->ExcludeClipRect(rcClient);

		if (SUCCEEDED(m_wrapperEdit.DrawThemeBackground(*pDC, 0, IsWindowEnabled() ? ETS_NORMAL : ETS_DISABLED, &rc, NULL)))
		{
...
void CXTPEdit::DrawBuddyBorders(CWnd* pBuddy, CDC* pDC, CRect rc, int nAlignemnt)
{
	...
	if (m_bUseVisualStyle && m_wrapperEdit.IsAppThemed())
	{
		...
		if (SUCCEEDED(m_wrapperEdit.DrawThemeBackground(*pDC, 0, ETS_NORMAL, &rc, NULL)))
		{
			return;
		}
	}
