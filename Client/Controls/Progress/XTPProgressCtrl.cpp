...
void CXTPProgressCtrl::RefreshMetrics()
{
	...
	m_wrapperProgress.OpenTheme(0, L"PROGRESS");
...
void CXTPProgressCtrl::DrawNcBorders(CDC* pDC, CRect rc)
{
	...
	if (m_bUseVisualStyle && m_wrapperProgress.IsAppThemed())
		return;
...
void CXTPProgressCtrl::DoPaint(CDC* pDC)
{
	...
	BOOL bThemed = m_bUseVisualStyle && m_wrapperProgress.IsAppThemed();
	...
	if (bThemed)
	{
		m_wrapperProgress.DrawThemeBackground(*pDC, bVertical ? PP_BARVERT : PP_BAR, 0, rcClient, NULL);
	}
	...
	if (bMarquee)
	{
		...
		if (bThemed)
		{
			m_wrapperProgress.GetThemeInt(0, 0, TMT_PROGRESSCHUNKSIZE, &dxBlock);
			m_wrapperProgress.GetThemeInt(0, 0, TMT_PROGRESSSPACESIZE, &dxSpace);
		}
		...
		for (int i = 0; i < 5; i++)
		{
			...
			if (bThemed)
			{
				m_wrapperProgress.DrawThemeBackground(*pDC, PP_CHUNK, 0, rc, NULL);
			}
			else
			{
	...
	else
	{
		...
		for (int i = 0; i < nBlocks; i++)
		{
			...
			if (bThemed)
			{
				m_wrapperProgress.DrawThemeBackground(*pDC, bVertical ? PP_CHUNKVERT : PP_CHUNK, 0, rc, NULL);
			}
			else
			{
				pDC->FillSolidRect(&rc, m_clrBarColor);
...
{
	if (m_bUseVisualStyle && m_wrapperProgress.IsAppThemed() && m_wrapperProgress.IsAppThemeReady())
	{
		return (BOOL)Default();
	}
	return TRUE;
