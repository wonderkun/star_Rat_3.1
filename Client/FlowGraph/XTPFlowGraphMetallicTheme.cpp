...
void CXTPFlowGraphMetallicTheme::DrawNode(CXTPFlowGraphDrawContext* pDC, CXTPFlowGraphNode* pNode)
{
	...
	pDC->SelectPen(clrFrame, 1);
	pDC->SelectFont(&m_lfText);
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		{
			pDC->SelectBrush(pPoint->IsSelected() ? clrSelection : pPoint->GetConnectionsCount() > 0 ? RGB(250,252,58) : RGB(182,182,182));
