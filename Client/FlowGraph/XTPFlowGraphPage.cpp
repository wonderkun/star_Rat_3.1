...
void CXTPFlowGraphPage::Reposition(CXTPFlowGraphDrawContext* pDC, CRect rcPage)
{
	...
	for (i = 0; i < nNodesCount; i++)
	{
		...
		for (int j = 0; j < pNode->GetConnectionPoints()->GetCount(); j++)
			pNode->GetConnectionPoints()->GetAt(j)->m_nConnectionsCount = 0;
	...
	for (i = 0; i < m_pConnections->GetCount(); i++)
	{
		...
		if (pConnection->GetInputPoint())
			pConnection->GetInputPoint()->m_nConnectionsCount++;

		if (pConnection->GetOutputPoint())
			pConnection->GetOutputPoint()->m_nConnectionsCount++;
	}
