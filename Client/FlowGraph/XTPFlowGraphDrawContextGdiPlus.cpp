...
void CXTPFlowGraphDrawContextGdiPlus::SelectBrush(COLORREF clr)
{
	m_pActiveBrush = CreateBrush(clr);
}

void CXTPFlowGraphDrawContextGdiPlus::SelectPen(COLORREF clrPen, int nWidth)
{
	SAFE_DELETE(m_pActivePen);
...
void CXTPFlowGraphDrawContextGdiPlus::SelectFont(LOGFONT* lf)
{
	SAFE_DELETE(m_pActiveFont);
	if (!lf)
		return;
