...
void CXTPMarkupDeviceContext::DrawPath(CXTPMarkupPathGeometry* pGeometry, CXTPMarkupStrokeStyle* pStrokeStyle, CXTPMarkupBrush* pFillBrush)
{
	int nCount = pGeometry->GetCoint();
	if (nCount == 0)
		return;

	::BeginPath(m_hDC);

	const MARKUP_POINTF* pPoints = pGeometry->GetPoints();
	const BYTE* pTypes = pGeometry->GetTypes();
...
void CXTPMarkupDeviceContext::DrawRectangle(CRect rc, CXTPMarkupBrush* pBrush, CXTPMarkupThickness* pThickness)
{
	...
	if (pThickness->left > 0)
		FillRectangle(CRect(rc.left, rc.top, rc.left + pThickness->left, rc.bottom), pBrush);

	if (pThickness->right > 0)
		FillRectangle(CRect(rc.right - pThickness->right, rc.top, rc.right, rc.bottom), pBrush);

	if (pThickness->top > 0)
		FillRectangle(CRect(rc.left, rc.top, rc.right, rc.top + pThickness->top), pBrush);

	if (pThickness->bottom > 0)
		FillRectangle(CRect(rc.left, rc.bottom - pThickness->bottom, rc.right, rc.bottom), pBrush);
}
...
void CXTPMarkupDeviceContext::FillRoundRectangle(CRect rc, CXTPMarkupBrush* pBrush, double* pCornerRadius)
{
	...
	CXTPMarkupPathGeometry* pGeometry = builder.CreateGeometry();
	pGeometry->m_nPixelOffsetMode = 4;

	CXTPMarkupStrokeStyle style;
	memset(&style, 0, sizeof(style));

	DrawPath(pGeometry, &style, pBrush);
...
void CXTPMarkupGdiPlusDeviceContext::DrawPath(CXTPMarkupPathGeometry* pGeometry, CXTPMarkupStrokeStyle* pStrokeStyle, CXTPMarkupBrush* pFillBrush)
{
	GpGraphics* pGpGraphics = GetGraphics();
	if (!pGpGraphics)
	{
		CXTPMarkupDeviceContext::DrawPath(pGeometry, pStrokeStyle, pFillBrush);
		return;
	}

	GetGdiPlus()->GdipSetSmoothingMode(pGpGraphics, SmoothingModeHighQuality);

	if (pGeometry->GetCoint() > 0)
	{
		GpPath* pGpPath = NULL;
		GetGdiPlus()->GdipCreatePath2((GpPointF*)pGeometry->GetPoints(), pGeometry->GetTypes(), pGeometry->GetCoint(), FillModeAlternate, &pGpPath);
		...
		if (pGpPath)
		{
			if (pFillBrush)
			{
				...
				GpBrush* pGpBrush = CreateGpBrush(pFillBrush, pGeometry->GetBounds());
			...
			{
				if (pGeometry->m_nPixelOffsetMode == 0)
					GetGdiPlus()->GdipSetPixelOffsetMode(pGpGraphics, PixelOffsetModeHalf);
				else
					GetGdiPlus()->GdipSetPixelOffsetMode(pGpGraphics, (PixelOffsetMode)pGeometry->m_nPixelOffsetMode);

				GpPen* pGpPen = CreateGpPen(pStrokeStyle, pGeometry->GetBounds());
...
void CXTPMarkupGdiPlusDeviceContext::DrawRectangle(CRect rc, CXTPMarkupBrush* pBrush, CXTPMarkupThickness* pThickness)
{
	...
	if (pThickness->left > 0)
		GetGdiPlus()->GdipFillRectangleI(pGpGraphics, pGpBrush, rc.left, rc.top,  pThickness->left, rc.Height());

	if (pThickness->right > 0)
		GetGdiPlus()->GdipFillRectangleI(pGpGraphics, pGpBrush, rc.right - pThickness->right, rc.top, pThickness->right, rc.Height());

	if (pThickness->top > 0)
		GetGdiPlus()->GdipFillRectangleI(pGpGraphics, pGpBrush, rc.left + pThickness->left, rc.top, rc.Width() - pThickness->left - pThickness->right, pThickness->top);

	if (pThickness->bottom > 0)
		GetGdiPlus()->GdipFillRectangleI(pGpGraphics, pGpBrush, rc.left + pThickness->left, rc.bottom - pThickness->bottom, rc.Width() - pThickness->left - pThickness->right, pThickness->bottom);
