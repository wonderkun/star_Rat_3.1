...
CSize CXTPMarkupFrameworkElement::MeasureCore(CXTPMarkupDrawingContext* pDC, CSize szAvailableSize)
{
	...
	long num = pMargin->left + pMargin->right;
	long num2 = pMargin->top + pMargin->bottom;
...
BOOL CXTPMarkupFrameworkElement::GetLayoutClip(CRect& rc) const
{
	...
	long num3 = pMargin->left + pMargin->right;
	long num4 = pMargin->top + pMargin->bottom;
...
void CXTPMarkupFrameworkElement::ArrangeCore(CRect rcFinalRect)
{
	...
	offset.x += rcFinalRect.left + pMargin->left;
	offset.y += rcFinalRect.top + pMargin->top;
